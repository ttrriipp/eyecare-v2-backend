...
class AppointmentRequestForm
{
    ...
    {
        // ── Sidebar: patient context and resolution details ────────────────
        ...
        $linkedPatientInformation = Section::make('Patient Information')
        ...
        // ── Main (2/3): request workflow and matching ─────────────────────
        // ── 2. Potential Matches ──────────────────────────────────────────
        // Keep candidate review with the primary request content because it
        // supports the Link-to-Patient action.
        $potentialMatches = Section::make('Potential Matches')
            ->visible(fn ($record): bool => ($record?->hasIdentitySnapshot() ?? false) && $record?->patient_id === null)
            ->schema([
        ...
        // ── 4. Referral Source ─────────────────────────────────────────────
        $referral = Section::make('Referral')
        ...
        return $schema->columns(1)->components([
            Grid::make(3)->schema([
                ...
                Grid::make(1)->columnSpan(1)->schema([
                    $linkedPatientInformation,
                    $patientSnapshotInformation,
                    $referral,
                    $decisionDetails,
                ]),
            ]),
