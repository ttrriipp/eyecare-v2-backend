...
- **`product_type`** controls form behavior and API visibility. Fixed values: `frame`, `lens`, `contact_lens`, `accessory`. `frame` shows AR fields. Optical `lens` products use lens categories and staff assignment and are never directly orderable. `contact_lens` products are directly orderable without optical-lens assignment behavior. Solutions, cases, cleaning kits, and similar products use `accessory`. Mobile API returns `frame`, `contact_lens`, and `accessory` products (not `lens`). **Disabled on edit** — set at creation time only.
...
POST   /orders                  Submit order request (status locked to requested). `items[].lens_category_id` is nullable — omit when no optical lens assignment is needed. `items[].lens_type_id` accepted as a backward-compat alias (same meaning). `product_variant_id` must reference a `frame`, `contact_lens`, or `accessory` product — `lens` products are rejected.
...
**GET /products** (paginated, frame + contact lens + accessory):
```json
{
  "data": [{
    "id": 3,
...
- **Product type expansion:** `product_type` has 4 values: `frame`, `lens`, `contact_lens`, `accessory`. Frame and optical-lens special behavior is unchanged. Contact lenses and accessories are distinct directly orderable catalog types; solutions, cases, and cleaning kits use `accessory`. A forward migration blocks deployment if ambiguous legacy `general` rows still exist so they can be explicitly reclassified.
