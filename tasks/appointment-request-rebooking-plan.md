...
**Status:** Implemented on 2026-09-09; pending-request schedule edits added
...
second appointment. An owned patient may also revise the preferences on the
same pending request in place; this does not create another request or move
the appointment.
...
4. **Existing patient API shape, additive fields.** Keep the existing list,
   ...
   optional submission field, response metadata, and pending-request PATCH
   needed to identify and revise a linked request. Remove the direct patient
   reschedule route during cutover.
...
- An owner can revise an unexpired pending request through
  `PATCH /appointment-requests/{id}` without changing its ID, number, patient,
  type, duration, reason, or identity.
...
- The pending-request PATCH documents future/grid/availability validation,
  linked appointment exclusion, expiry recalculation, and stable 422 codes.
...
- Pending request schedule edits are in-place and do not reserve capacity.
